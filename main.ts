type Level = {
  ...
  requirements?: {
    // add any specific level requirements here
  }
};
...
let currentLevel = 0;
let startPosition: number = levels[currentLevel].startPosition;
let lastClickedElement: number;
let secondToLastClickedElement: number = -1;
...
Array.from(elements).forEach(element => {
  ...
  element.addEventListener("click", function () {
    ...
    if (fillElement) {
      ...
      let elementQuery: string = `[elementNumber="${lastClickedElement}"]`;
      var previousElement = document.querySelector(elementQuery);
      if (secondToLastClickedElement == -1) {
        //place and rotate tail image
        (previousElement.firstChild as HTMLImageElement).src = 'Tail.png';
        switch (direction) {
          case 1:
            (previousElement.firstChild as HTMLImageElement).style.transform = 'rotate(180deg)';
            break;
          case 2:
            (previousElement.firstChild as HTMLImageElement).style.transform = 'rotate(270deg)';
            break;
          case 3:
            break;
          case 4:
            (previousElement.firstChild as HTMLImageElement).style.transform = 'rotate(90deg)';
            break;
        }
      } else {
        if (previousDirection == direction) {
          (previousElement.firstChild as HTMLImageElement).src = 'Straight.png';
          ...
          (previousElement.firstChild as HTMLImageElement).src = 'Corner.png';
          if ((previousDirection == 1 && direction == 2) || (previousDirection == 4 && direction == 3)) {
            ...
            (previousElement.firstChild as HTMLImageElement).style.transform = '';
